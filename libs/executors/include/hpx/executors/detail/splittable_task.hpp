...
namespace hpx { namespace parallel { namespace execution {
    ...
    private:
        void call_idle_mask()
        {
            ...
            std::size_t remainder = static_cast<std::size_t>(std::floor(
                (stop_ - start_) * double(num_free_) / double(num_free_ + 1)));
            std::size_t task_size = static_cast<std::size_t>(
                std::ceil((stop_ - start_) / double(num_free_ + 1)));
        ...
        void call_all_multiple_tasks()
        {
            ...
            if (num_free_ != 0 && task_size != 0)
            {
                ...
                for (std::size_t i = 0; i != num_free_ - 1; ++i)
                {
                    using policy = hpx::launch::async_policy;
